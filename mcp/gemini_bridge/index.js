...
async function isBinaryFile(filePath, bytesToCheck = 8192) {
    try {
...
async function fetchAvailableModels(apiKey) {
    try {
        ...
        return await loadFallbackModels();
    }
}

// Load models from local models.md file as fallback
async function loadFallbackModels() {
    try {
        const modelsDoc = await readFile(join(process.cwd(), 'models.md'), 'utf-8');
        // Extract model names from the documentation
        const models = [
            'gemini-2.0-flash-exp',
            'gemini-1.5-flash',
            'gemini-1.5-flash-8b',
        ...
        console.error(`📋 Using fallback model list (${models.length} models)`);
        return models;
    } catch (error) {
        console.error(`⚠️  Could not load fallback models: ${error.message}`);
        return ['gemini-2.0-flash-exp']; // Ultimate fallback
    }
}
...
function createSmartError(error, context = {}) {
...
function optimizeContext(fileContents, maxTokens) {
...
function estimateTokensForFiles(fileContents) {
    let totalChars = 0;
    ...
    setupTools() {
        ...
        this.server.tool(
            ...
            async ({pattern_type, files, specific_focus, model}) => {
                ...
                // Reuse the send_to_gemini logic
                return await this.server.callTool("send_to_gemini", {
                ...
                if (include_models) {
                    try {
                        ...
                        modelInfo = `\n\n**Available Models (${models.length}):**\n` +
                            ...
                            `\n\n📊 **Model Cache:** ${modelCache.fallbackUsed ? 'Using fallback list' : 'Live from API'} (updated ${Math.round((Date.now() - modelCache.timestamp) / 60000)}m ago)`;
                    } catch (error) {
